...
foreach ($name in @('yt-dlp', 'ffmpeg', 'deno', 'inno')) {
    ...
    if ($name -eq 'yt-dlp') {
        ...
        $expanded = Join-Path $toolRoot 'archives/ffmpeg-expanded'
        Expand-Archive -LiteralPath $archive -DestinationPath $expanded -Force
        $distribution = Get-ChildItem -LiteralPath $expanded -Directory | Select-Object -First 1
        Copy-Item -Path (Join-Path $distribution.FullName 'bin/*') -Destination $destination -Force
        Copy-Item -Path (Join-Path $distribution.FullName '*') -Destination $destination -Recurse -Force
    }
...
$licenses = @{
    ...
    'FFmpeg-GPLv3.txt' = 'https://raw.githubusercontent.com/FFmpeg/FFmpeg/5c8e7e2433/COPYING.GPLv3'
