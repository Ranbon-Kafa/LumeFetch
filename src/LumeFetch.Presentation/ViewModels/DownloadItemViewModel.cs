...
public sealed class DownloadItemViewModel : ObservableObject
{
    ...
    public bool CanUsePrimaryAction => !_snapshot.RequiresReview && _snapshot.Status is DownloadStatus.Downloading or DownloadStatus.Paused or DownloadStatus.Failed or DownloadStatus.Canceled;
    ...
    public string ProgressText
    {
        get
        {
            ...
            {
                return _snapshot.OutputDisplayPath ?? _snapshot.OutputPath ?? Localizer.Current["Completed"];
            }

            if (_snapshot.Status == DownloadStatus.Failed)
            {
                return Localizer.Current[_snapshot.ErrorMessage ?? "Failed"];
            }
